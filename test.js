 ...
 const product = [
    ...
    // const newData =  [...new Set([...product.map((p)=>p.category)])];

    // const existingData = newData.map((c)=>({value:c,label:c.split("-").join(" "),checked:false}));
    // console.log(existingData);
    

const Brands = [...new Set([...product.map((p)=>p.brand)])]
console.log(Brands);
const existingBrand = Brands.map((c)=>({value:c,label:c.split("-").join(" "),checked:false}));
